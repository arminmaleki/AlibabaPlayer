...
public class Synth1 implements Synth {
	String name="";
	AudioContext ac;
	float vib=0;
...
if (ps.get("tone")!=null) toneGl=ps.get("tone"); else toneGl=new Glide(ac,0,1000);
       WavePlayer vwp=new WavePlayer(ac,(float)8.0,Buffer.SINE);
...
// 	      WavePlayer w=(WavePlayer) vwp;
 //	      int a, ind;
      ...
      // WavePlayer wp=new WavePlayer(ac, (float)(220*Math.exp(N.pitch*Math.log(2)/12)), Buffer.SINE);
 	   WavePlayer wp=new WavePlayer(ac, function1, Buffer.SINE);
...
//	      int a, ind;
 ...
 //      for (String s: vhash.values()) System.out.print(s + " ");
	...
	public Synth1(String name,AudioContext ac,float vib, float tail){
		this.name=name;
		this.ac=ac;
		this.vib=vib;
		this.tail=tail;
